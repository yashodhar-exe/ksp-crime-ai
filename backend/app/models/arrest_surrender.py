...
class ArrestSurrender(Base):
    ...
    arrest_surrender_type_id: Mapped[int | None] = mapped_column(Integer, nullable=True)  # lookup: arrest / surrender
    arrest_surrender_date: Mapped[date | None] = mapped_column(Date, nullable=True)
    arrest_surrender_state_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("state.state_id"), nullable=True)
    ...
    io_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("employee.employee_id"), nullable=True)
    court_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("court.court_id"), nullable=True)
    accused_master_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("accused.accused_master_id"), nullable=True)
    ...
    accused: Mapped["Accused | None"] = relationship(back_populates="arrest_surrenders")  # noqa: F821
